...
import type { MailtrapThreadId } from "./types.js";
...
export class ThreadResolver {
  ...
  encodeThreadId(id: MailtrapThreadId): string {
    return `mailtrap:${id.toAddress}:${id.rootMessageIdHash}`;
  }
    ...
    if (inReplyTo || references) {
      ...
      for (const candidate of candidateIds) {
        const existingThread = this.state
          ? await this.state.get<string>(`mailtrap:message-thread:${candidate}`)
  ...
  async trackMessage(threadId: string, messageId: string): Promise<void> {
    if (this.state) {
      const key = `mailtrap:thread-messages:${threadId}`;
      ...
      await this.state.set(
        `mailtrap:message-thread:${messageId}`,
        threadId,
        STATE_TTL_MS,
      );
      return;
  ...
  async getReplyHeaders(
    ...
    const messages = this.state
      ? await this.state.getList<string>(`mailtrap:thread-messages:${threadId}`)
  ...
  async trackSubject(threadId: string, subject: string): Promise<void> {
    if (this.state) {
      await this.state.setIfNotExists(
        `mailtrap:thread-subject:${threadId}`,
        subject,
        STATE_TTL_MS,
      );
      return;
  ...
  private extractMessageIds(
    ...
    if (Array.isArray(references)) {
      ids.push(...references.map((s) => s.trim()).filter(Boolean));
    } else if (references) {
      ...
      if (trimmed.startsWith("[")) {
        try {
          ...
          if (Array.isArray(parsed)) {
            ids.push(
              ...parsed.map((s: string) => String(s).trim()).filter(Boolean),
            );
          }
        } catch {
